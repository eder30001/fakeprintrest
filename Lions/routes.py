...
from Lions import app,database, bcrypt
from Lions.models import Usuario, Foto
from flask_login import login_required, login_user,logout_user,current_user
from Lions.forms import FormLogin, FormCriarConta, FormFoto
import os
...
def homepage():
    ...
    if formlogin.validate_on_submit():
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha,formlogin.senha.data):
            login_user(usuario)
            return redirect(url_for("perfil",id_user=usuario.id))
...
def perfil(id_user):
    if int(id_user)==int(current_user.id):
        ...
        user = Usuario.query.get(int(id_user))
...
@login_required
def feed():
    fotos= Foto.query.order_by(Foto.data_criacao.desc()
                               ).all()
